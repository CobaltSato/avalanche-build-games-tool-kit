...
このスターターキットは、**EIP-1193**という標準規格に準拠したウォレットをサポートしています。

**EIP-1193とは？** Ethereum Improvement Proposal 1193の略で、ブラウザ拡張機能のウォレットがWebアプリケーションとやり取りするための標準規格です。この規格に準拠しているウォレットは、同じインターフェースで使うことができます。

**主な対応ウォレット：**

- **MetaMask**: 最も一般的なブラウザ拡張機能のウォレットです
- **Avalanche Core Wallet**: Avalanche公式のウォレットです。MetaMaskと同じEIP-1193規格に準拠しているため、同じように使えます
- **その他のEIP-1193準拠ウォレット**: EIP-1193規格に準拠している他のウォレットも使用可能です

**なぜ複数のウォレットが使えるのか？**

MetaMaskとCore Walletは、どちらも同じEIP-1193規格に準拠しているため、同じインターフェース（`window.ethereum`）を通じてアクセスできます。そのため、このスターターキットでは、どちらのウォレットでも同じように動作します。

### ウォレット接続の流れ

1. **ユーザーが「接続」ボタンをクリック**
2. **ウォレット（MetaMaskやCore Walletなど）がポップアップを表示**して、接続の許可を求めます
3. **ユーザーが許可**すると、アプリがウォレットのアドレスを取得できます
4. **アプリはアドレスを使って**、ユーザー固有のデータを取得したり、操作を実行したりできます

**注意：** ユーザーのブラウザにインストールされているウォレットが自動的に検出されます。複数のウォレットがインストールされている場合、ブラウザが使用するウォレットを選択する画面が表示されることがあります。
...
- **`connectWallet`**: ウォレットを接続する関数です。呼び出すと、インストールされているウォレット（MetaMaskやCore Walletなど）のポップアップが表示されます
...
3. インストールされているウォレット（MetaMaskやCore Walletなど）のポップアップが表示される
4. ユーザーが許可すると、`isConnected` が `true` になり、`account` にアドレスが設定される

**どのウォレットが使われるか：**

ブラウザにインストールされているEIP-1193準拠のウォレットが自動的に検出されます。複数のウォレットがインストールされている場合、ブラウザが使用するウォレットを選択する画面が表示されることがあります。
...
2. ウォレット（MetaMaskやCore Walletなど）のポップアップが表示され、ユーザーがトランザクションを承認する
3. トランザクションがブロックチェーンに送信される
4. マイニングが完了するまで待機する（数秒〜数分かかる場合があります）
5. 成功すると、`txStatus` が `'success'` になる

**重要：** トランザクション送信にはガス代がかかります。ユーザーはウォレットでガス代を確認してから承認します。
...
- **ユーザーがトランザクションを拒否した場合**: ウォレットで「拒否」をクリックすると、エラーが発生します
...
| ウォレット | MetaMask / Core Wallet (EIP-1193) | EIP-1193規格に準拠したブラウザ拡張機能のウォレット。MetaMaskやAvalanche Core Walletなどが使用可能です |

**EIP-1193について：**

EIP-1193は、ブラウザ拡張機能のウォレットがWebアプリケーションとやり取りするための標準規格です。この規格に準拠しているウォレットは、同じインターフェース（`window.ethereum`）を通じてアクセスできるため、複数のウォレットを同じコードでサポートできます。

- **MetaMask**: 最も一般的なEIP-1193準拠ウォレット
- **Avalanche Core Wallet**: Avalanche公式のウォレット。MetaMaskと同じEIP-1193規格に準拠
- **その他のEIP-1193準拠ウォレット**: 同じ規格に準拠している他のウォレットも使用可能
...
- **EIP-1193準拠**: `MetaMaskAdapter` は、EIP-1193という標準規格に準拠したウォレットをサポートしています。そのため、MetaMaskだけでなく、Avalanche Core Walletなど、同じ規格に準拠した他のウォレットも同じアダプターで使用できます
- **インターフェースの配置**: `WalletAdapter` インターフェースは Wallet Package 内に定義されています。これにより、Wallet Package が自己完結的なパッケージとして設計されています
- **拡張性**: 新しいウォレットアダプターを追加する場合は、`WalletAdapter` インターフェースを実装するだけで対応できます。また、EIP-1193に準拠していないウォレットも、専用のアダプターを実装することで対応可能です
